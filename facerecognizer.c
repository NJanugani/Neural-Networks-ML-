...
extern char *strcpy();
extern void exit();
char* stri[20] = { "an2i", "at33","boland", "bpm", "ch4f", "cheyer", "choon", "danieln", "glickman", "karyadi", "kawamura", "kk49", "megak","mitchell", "night", "phoebe", "saavik", "steffi", "sz24","tammo"};
int totalcounter = 0;
int counter = 0;
double x=0;
double y=0;
...
char *netname;
{
  ...
  for (epoch = 1; epoch <= epochs; epoch++) {
    ...
    for (i = 0; i < train_n; i++) {
      ...
      /** Run backprop, learning rate0, momentum **/
      bpnn_train(net, 0.2, 0.2, &out_err, &hid_err);
